"""
조건: 클러스터링 먼저(전체 데이터) -> 그 후 필터링(평점 3.5이하), 인기순 고려, 태그 관련도 0.8
"""
...
class ColdStartMovieSelector:
    def __init__(self, 
                 ...
                 output_path='/home/ubuntu/ai-model/clustering/tag_movie/12-11/results/cluster_first/v2',
                 # --- 설정 ---
                 clustering_vote_threshold=100, # 클러스터링을 위한 최소 인기도 (노이즈 제거용)
                 final_min_vote_count=500,      # 최종 결과 필터링용 인기도
                 max_rating=3.5,                # 최종 결과 필터링용 평점 상한
                 num_clusters=12,
