...
"""
Hybrid Recommender with Genre, Runtime & OTT Filtering
...
class HybridRecommender:
    def __init__(
        ...
        self._load_ott_data(ott_path)  # 추가
        ...
        for movie_data in self.metadata_map.values():
            ...
            if isinstance(genres, str) and genres:
                try:
                    ...
                    # 공백 제거 및 유효한 장르만 추가
                    genre_list = [g for g in genre_list if g.strip()]
                    all_genres.update(genre_list)
                except:
                    pass
        
        # 공백 제거 및 정렬
        self.all_genres = sorted([g for g in all_genres if g.strip()])
    ...
    def _load_ott_data(self, path: str):
        ...
        # tmdbId를 키로, OTT 리스트를 값으로 하는 딕셔너리 생성
        self.movie_ott_map = {}
        for tmdb_id, group in df.groupby('tmdbId'):
            self.movie_ott_map[tmdb_id] = group['ott_name'].unique().tolist()
        
        # 전체 OTT 목록 추출
        self.all_otts = sorted(df['ott_name'].unique().tolist())
    ...
    def _apply_filters(
        ...
        preferred_otts: Optional[List[str]] = None  # 추가
...
if __name__ == "__main__":
    ...
    OTT_PATH = "/home/ubuntu/ai-model/datas/data/movie_ott_final.csv"  # 추가
