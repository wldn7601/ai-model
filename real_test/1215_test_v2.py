...
class HybridRecommender:
    ...
    def recommend(
        ...
        print(f"Filters - Genres: {preferred_genres}, OTT: {preferred_ott}, Max Runtime: {max_runtime}, Allow Adult: {allow_adult}")
...
def get_user_input_for_filters(recommender: HybridRecommender):
    ...
    runtime_input = input("\n선택 (1-5, 엔터로 제한 없음): ").strip()
    
    max_runtime = None
    runtime_map = {'1': 90, '2': 120, '3': 150, '4': 180, '5': None}
    if runtime_input in runtime_map:
        max_runtime = runtime_map[runtime_input]
    ...
    print(f"최대 런타임: {max_runtime if max_runtime else '제한 없음'}")
...
if __name__ == "__main__":
    ...
    for i, rec in enumerate(recommendations, 1):
        ...
        ott_str = ', '.join(ott_list[:2])  # 최대 2개만 표시
        if len(ott_list) > 2:
