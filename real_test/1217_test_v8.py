...
class HybridRecommender:
    ...
    def recommend(
        ...
        # 4. Track A 필터링 (장르 + 연도 적용)
        filtered_ids_a, filtered_indices_a = self._apply_filters(
            self.common_movie_ids, preferred_genres, max_runtime, min_year=2010
        )
        
        # 5. Track B 필터링 (장르 무시, 연도 적용)
        filtered_ids_b, filtered_indices_b = self._apply_filters(
            self.common_movie_ids, None, max_runtime, min_year=2010
        )
